...
Only exported changes survive a rebuild — anything edited in `merged/` but not exported is lost the next time `build.sh` runs.
...
| `npm run check` | Run the linters and the full test suite. |
...
| `build.sh [ref]` | Builds `merged/` (worktree of `upstream/`) with everything applied. Default ref: `UPSTREAM_BASE`. Conflicts stay as `<<<<<<< OSM upstream / OHM patch` markers. Needs `upstream/` to exist and does not fetch. |
| `export.sh <file>` \| `--all` | Saves edits in `merged/` back into `patches/`. `--all` exports everything modified and deletes obsolete patches. |
| `apply.sh <checkout>` | One-shot apply onto any checkout. Used by CI; exits non-zero on failure. No npm script. |
